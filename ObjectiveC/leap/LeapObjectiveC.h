 ...
 * Added ARC compiling check, declarations and autorelease calls in order to 
   be able to use the file in 32 bit MRC mode.
 * Moved the #import "Leap.h" and <string> directives to the .h file.
 * Passed LeapVector values to double instead of float, in order to be able 
   to use the native methods to get precise values from an interaction box
   converted to tiny coordinates (like -1.->1.).
*************************************************************************/
 ...
 */
- (double)magnitude;
@property (nonatomic, getter = magnitude, readonly)double magnitude;
/**
...
@property (nonatomic, getter = magnitudeSquared, readonly)double magnitudeSquared;
/**
 ...
 */
- (double)pitch;
@property (nonatomic, getter = pitch, readonly)double pitch;
/**
 ...
 */
- (double)roll;
@property (nonatomic, getter = roll, readonly)double roll;
/**
 ...
 */
- (double)yaw;
@property (nonatomic, getter = yaw, readonly)double yaw;
/**
 * Adds two vectors.
 *
...
typedef enum LeapPolicyFlag {
    ...
    LEAP_POLICY_IMAGES = (1 << 1),            /**< Receive raw images from sensor cameras. */
    LEAP_POLICY_OPTIMIZE_HMD = (1 << 2),      /**< Optimize the tracking for head-mounted device. */
} LeapPolicyFlag;
